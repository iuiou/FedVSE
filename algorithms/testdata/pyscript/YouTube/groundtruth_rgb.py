...
if __name__ == '__main__':
    ...
    meta_list = []
    ans = []
    qurey, conds = read_query("res/YouTube-rgb/query.txt")
    heaps = [MaxHeap() for _ in range(len(qurey))]
    for j in range(silos):
        data_tmp = []
        read_fivecs(f"res/YouTube-rgb/YouTube_{j}.fivecs", data_tmp)
        data_list.extend(data_tmp)
        meta_type, content = read_meta(f"res/YouTube-rgb/meta_{j}.txt")
    ...
    for x in range(len(qurey)):
        ...
        tmp.reverse()
        ans.append(tmp)
    dump_ground_truth_to_ivecs(f"res/YouTube-rgb/gt_{kkk}_{silos}.ivecs", ans)
