...
def sta(cond, m):
    _cond = str(cond[0])
    _cond = _cond.replace("=", "==")
    _cond = _cond.replace("label", '"' + str(m) + '"')
    return eval(_cond)
...
if __name__ == '__main__':
    ...
    for j in range(silos):
        data_tmp = []
        read_fivecs(f"res/YouTube-audio/YouTube_{j}.fivecs", data_tmp)
        data_list.extend(data_tmp)
        meta_type, content = read_meta(f"res/YouTube-audio/meta_{j}.txt")
        meta_list.extend(content)
    # 读取query
    qurey, conds = read_query("res/YouTube-audio/query.txt")
    for x in range(len(qurey)):
        ...
        tmp.reverse()
        ans.append(tmp)
    dump_ground_truth_to_ivecs(f"res/YouTube-audio/gt_{kkk}_{silos}.ivecs", ans)
